...
================================================
    ...
    def process_synced(self):
        ...
        preprocessed_prompt = self._preprocess_prompt(self.user_prompt, adapt_to_pi=True)
        
        obs = {
                "images": {
                    "1": imgs[0], 
                    "2": imgs[1],
                    "3": black_img},  # 右臂使用黑色占位图像
        ...
        processed_prompt = self.postprocess_actions(result["prompt"][0], adapt_to_pi=True)
    ...
    def get_arm_state(self, campare_stamp):
        ...
        try:
            ...
            preprocessed_state = self._preprocess_joint_state(state[0], adapt_to_pi=True)
    ...
    def postprocess_actions(self, actions, adapt_to_pi=True):
        ...
        if "actions" in self.norm_stats and "mean" in self.norm_stats["actions"] and "std" in self.norm_stats["actions"]:
            action_mean = np.array(self.norm_stats["actions"]["mean"])[:14]  # 只使用前14个维度
            action_std = np.array(self.norm_stats["actions"]["std"])[:14]    # 只使用前14个维度
