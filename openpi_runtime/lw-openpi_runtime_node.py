...
# 确保能正确导入本地模块和openpi_runtime包
import os
# 将项目根目录添加到sys.path，这样能找到openpi_runtime包
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# 将当前目录添加到sys.path，这样能找到本地模块
...
class OpenpiRuntimeNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('action_pub_frequency', 50.0)  # Hz
...
================================================
    ...
    def process_synced(self):
        ...
        # 检查result是否为None
        if result is None:
            self.get_logger().error("未收到来自服务器的有效响应")
            return
        
        # 调试日志：打印result["prompt"]的结构和长度
        self.get_logger().info(f'接收到的result["prompt"]结构: 外层长度={len(result["prompt"])}, 内层长度={len(result["prompt"][0])}')
        
        # 遍历所有动作序列
        for i, prompt in enumerate(result["prompt"][0]):
            self.get_logger().info(f'处理第{i+1}/{len(result["prompt"][0])}个动作序列')
