...
def set_joint_value(position, piper, gripper):
    ...
    piper.GripperCtrl(joint_6, 3000, 0x01, 0)
    # time.sleep(0.005)
...
def main(args):
    ...
    try:
        ...
        while True:
            ...
            alicia_joint_pos = robot.get_joints()
            gripper_value = robot.get_gripper()

            piper_joint_pos = joint_projection(alicia_joint_pos)
            set_joint_value(piper_joint_pos, piper, gripper_value)
            time.sleep(0.02)
